...
// circular partitioned array, an alternative for std::vector<std::vector<T>> that's more cache friendly and memory efficient
...
private:
	...
	void push_back_impl(u32 partIndex, const T& val) {
		...
		data[clampIndex(attrib.offset + attrib.len++)] = val;
		m_totalSize++;
	}
};
