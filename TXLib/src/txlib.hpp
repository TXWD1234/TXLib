...
namespace tx {
	...
	public:
		...
		constexpr inline bool operator!=(const vec2& other) const { return !(this->m_x == other.m_x && this->m_y == other.m_y); }
	...
	public:
		...
		constexpr inline Coord operator=(const Coord& other) { this->m_x = other.m_x; this->m_y = other.m_y; return other; }
	...
	constexpr inline float sign(float num) { return (num == 0 ? 1 : num / std::fabs(num)); }
	...
	constexpr inline vec2 unify(vec2 in, float dist) {
		if (dist == 0.0f) return vec2(0, 0);
		return vec2(in.getX() / dist, in.getY() / dist);
	}
	constexpr inline vec2 reverse(const vec2& in) { return vec2(in.x() * -1.0f, in.y() * -1.0f); }
...
#undef max
	...
	inline vec2 leftPerp (const vec2& in) { return vec2{ -in.y(), in.x() }; }
	inline vec2 rightPerp(const vec2& in) { return vec2{ in.y(), -in.x() }; }
...
#define ONE_DEGREE 0.017453292f
	...
	template<class T>
	inline bool inRange(T val, T min, T max) { // inclusive
...
#undef RGB
	...
	constexpr RGB InvalidColor = RGB(-1, -1, -1);
